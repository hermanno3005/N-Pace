...
- **FR-8.2** MUST report fit quality (R², residual spread) and never silently overfit on sparse data; fall back to the coefficients in force when data is insufficient — the installation's own where `pacelab.toml` supplies them, the population defaults otherwise (ADR-0021).
